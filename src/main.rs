...
use crate::sandstorm_custom_transport::{
    SANDSTORM_RAW_UDP_TRANSPORT_ID, socket_addr_to_custom_addr,
};
use crate::storage::{
    LocalProxyCapabilityRecord, LocalProxyTargetKind, ReceivedCapabilityKind,
    RegisteredRemoteCapabilityDurability, RegisteredRemoteCapabilityRecord, SavedCapabilityRecord,
...
fn encode_json<T: Serialize>(value: &T) -> Result<Vec<u8>, String> {
    serde_json::to_vec(value).map_err(|err| format!("failed to encode json response: {err}"))
}
...
impl grain_capnp::ui_view::Server for UiViewImpl {
    fn get_view_info(
        ...
        let match_request_descriptors = collect_match_request_descriptors(&self.app_state)
            .unwrap_or_else(|err| {
        ...
        let mut match_requests = view_info
            .reborrow()
        ...
        for (index, encoded) in match_request_descriptors.iter().enumerate() {
            ...
            let Ok(descriptor) =
                message.get_root::<powerbox_capnp::powerbox_descriptor::Reader<'_>>()
            else {
                continue;
            };
            if let Err(err) =
                copy_powerbox_descriptor(descriptor, match_requests.reborrow().get(index as u32))
            {
...
impl grain_capnp::main_view::Server<text::Owned> for UiViewImpl {
    fn restore(
        ...
        Promise::from_future(async move {
            ...
            let restored_cap = restore_app_object_capability(sandstorm_api, &app_state, &object_id)
                .await
                .map_err(|err| {
                    eprintln!(
            ...
            results.get().get_cap().set_as_capability(restored_cap.hook);
            Ok(())
        })
    }
    ...
    fn drop(
        ...
        Promise::from_future(async move {
            ...
            match drop_received_remote_capability(&app_state, &object_id) {
                Ok(true) => {
                    eprintln!(
                        "main_view.drop: object_id={object_id} dropped at_ms={}",
                        now_ms()
                    );
                    Ok(())
                }
...
impl web_session_capnp::web_session::Server for WebSessionImpl {
        ...
        if path == "api/state" {
            ...
            return Promise::from_future(async move {
                if let Err(err) = refresh_peer_rpc_exports(&app_state).await
                    && err != "peer rpc session is not connected"
                {
                    eprintln!("state refresh of peer exports failed: {err}");
                }

                let mut body = match render_state_value(&app_state) {
                    Ok(body) => body,
                    Err(err) => {
    ...
    fn post(
        ...
        if path == "api/pairing/remote-ticket" {
            ...
            match outcome {
                Ok(()) => {
                    let body = match encode_json(&serde_json::json!({ "ok": true })) {
                        Ok(body) => body,
                        Err(err) => {
                            let mut error = results.get().init_server_error();
                            error.set_description_html(
        ...
        if path == "api/pairing/probe-connect" {
            ...
            return Promise::from_future(async move {
                match probe_remote_connection(app_state).await {
                    Ok(summary) => {
                        let body = match encode_json(&serde_json::json!({
                            "ok": true,
                            "remoteNodeId": summary.remote_node_id,
            ...
            return Promise::from_future(async move {
                tokio::task::spawn_local(async move {
                    if let Err(err) = begin_pair_connection(app_state.clone(), sandstorm_api).await
                        && let Ok(mut guard) = app_state.lock()
                    {
                ...
                let body = match encode_json(&serde_json::json!({ "ok": true })) {
                    ...
                    Err(err) => {
                        ...
                        error.set_description_html(
                            format!(
                                "<!doctype html><title>Tunnel Connect Failed</title><pre>{}</pre>",
                                escape_html(&err)
                            )
                            .as_str(),
                        );
            ...
            return Promise::from_future(async move {
                ...
                let body = match encode_json(&serde_json::json!({ "ok": true })) {
                    ...
                    Err(err) => {
                        ...
                        error.set_description_html(
                            format!(
                                "<!doctype html><title>Accept Connection Failed</title><pre>{}</pre>",
                                escape_html(&err)
                            )
                            .as_str(),
                        );
            ...
            return Promise::from_future(async move {
                ...
                let body = match encode_json(&serde_json::json!({ "ok": true })) {
                    ...
                    Err(err) => {
                        ...
                        error.set_description_html(
                            format!(
                                "<!doctype html><title>Enable Tunnel Failed</title><pre>{}</pre>",
                                escape_html(&err)
                            )
                            .as_str(),
                        );
        ...
        if path != "api/saved-cap/restore" {
            if path == "api/saved-cap/resolve-object" {
                ...
                return Promise::from_future(async move {
                    ...
                    let label = match capability_label_for_object_id(
                        &app_state,
                        &effective_object_id,
                    ) {
                        Ok(label) => label,
                ...
                let session_context = self.session_context.clone();
                return Promise::from_future(async move {
                    let (_label, object_id) = match create_local_proxy_for_remote_export(
                        &app_state, &export_id,
                    )
                    .await
                    {
                    ...
                    let descriptor_source = match capability_descriptor_for_object_id(
                        ...
                        Ok(Some(value)) => {
                            descriptor_json_to_b64(&value).unwrap_or(request_descriptor_b64.clone())
                        }
...
fn load_shared_capabilities() -> Result<Vec<SharedCapability>, String> {
    app_storage()
        ...
        .into_iter()
        .map(
            |row: SharedCapabilityRecord| -> Result<SharedCapability, String> {
                let saved_cap =
...
fn load_registered_remote_capabilities() -> Result<Vec<RegisteredRemoteCapability>, String> {
    Ok(app_storage()
        ...
        .into_iter()
        .map(
            |record: RegisteredRemoteCapabilityRecord| RegisteredRemoteCapability {
                ...
                type_tag: record.type_tag.unwrap_or_else(|| {
                    imported_type_tag_for_kind(ImportedRemoteCapabilityKind::Other)
                }),
                descriptor_json: record.descriptor_json,
...
fn imported_kind_to_tunnel_kind(
    kind: ImportedRemoteCapabilityKind,
) -> tunnel_capnp::CapabilityKind {
    match kind {
...
impl AppState {
    fn initialize() -> Result<Self, String> {
        ...
        if had_remote_object_local_proxies {
            local_proxy_caps
                .retain(|record| record.target_kind == LocalProxyTargetKindRuntime::ExportId);
...
async fn initialize_iroh_endpoint(
    ...
    let (raw_udp_interface, raw_udp_interface_source) = load_configured_raw_udp_interface_state()?;
    {
        let mut guard = app_state
            .lock()
            .map_err(|_| "app state lock poisoned".to_string())?;
...
fn load_configured_raw_udp_interface_state()
-> Result<(Option<SavedCapability>, Option<String>), String> {
...
fn load_saved_raw_udp_port() -> Result<Option<u16>, String> {
    ...
    match std::fs::read_to_string(&path) {
        ...
        Err(err) if err.kind() == std::io::ErrorKind::NotFound => Ok(None),
        Err(err) => Err(format!(
            "failed to read raw udp port {}: {err}",
            path.display()
        )),
    }
}
...
fn persist_raw_udp_interface_token(saved_token: &str) -> Result<(), String> {
    ...
    std::fs::write(&path, format!("{saved_token}\n")).map_err(|err| {
        format!(
            "failed to persist raw udp interface token {}: {err}",
            path.display()
        )
    })
}
...
fn clear_persisted_raw_udp_interface_token() -> Result<(), String> {
    ...
    match std::fs::remove_file(&path) {
        ...
        Err(err) => Err(format!(
            "failed to clear raw udp interface token {}: {err}",
            path.display()
        )),
    }
}
...
fn clear_persisted_raw_udp_port() -> Result<(), String> {
    ...
    match std::fs::remove_file(&path) {
        ...
        Err(err) if err.kind() == std::io::ErrorKind::NotFound => Ok(()),
        Err(err) => Err(format!(
            "failed to clear raw udp port {}: {err}",
            path.display()
        )),
    }
}
...
async fn configure_raw_udp_interface_binding<Validate, ValidateFut, Rebind, RebindFut>(
...
async fn bind_local_iroh_endpoint(
    ...
    if let Some(config) = load_sandstorm_raw_udp_bind_config()? {
        eprintln!(
            ...
            config.interface_token_hex, config.port
        );
...
fn summarize_endpoint_addr_with_raw_udp_fallback(
    ...
    if summary.custom_addrs.is_empty() {
        ...
        summary.custom_addrs.push(
            socket_addr_to_custom_addr(SANDSTORM_RAW_UDP_TRANSPORT_ID, advertised_addr).to_string(),
        );
    }
...
fn load_remote_ticket() -> Result<Option<String>, String> {
    ...
    match std::fs::read_to_string(&path) {
        ...
        Err(err) if err.kind() == std::io::ErrorKind::NotFound => Ok(None),
        Err(err) => Err(format!(
            "failed to read remote ticket {}: {err}",
            path.display()
        )),
    }
}
...
fn load_tunnel_enabled() -> Result<bool, String> {
    ...
    match value.as_str() {
        ...
        "false" => Ok(false),
        _ => Err(format!(
            "failed to parse persisted tunnel enabled flag: {value:?}"
        )),
    }
}
...
fn clear_persisted_remote_ticket() -> Result<(), String> {
    ...
    match std::fs::remove_file(&path) {
        ...
        Err(err) if err.kind() == std::io::ErrorKind::NotFound => Ok(()),
        Err(err) => Err(format!(
            "failed to clear remote ticket {}: {err}",
            path.display()
        )),
    }
}
...
fn render_state_value(app_state: &Arc<Mutex<AppState>>) -> Result<serde_json::Value, String> {
    app_core(app_state).render_state_value()
}
...
fn make_powerbox_query_from_descriptor_b64(encoded: &str) -> Option<String> {
    ...
    let mut query_tags = query.reborrow().init_tags(tags.len());
    for index in 0..tags.len() {
        let tag = tags.get(index);
        query_tags.reborrow().get(index).set_id(tag.get_id());
    }
...
fn copy_powerbox_descriptor(
    ...
    if let Ok(tags) = source.get_tags() {
        let mut target_tags = target.reborrow().init_tags(tags.len());
        for index in 0..tags.len() {
            let tag = tags.get(index);
            let mut target_tag = target_tags.reborrow().get(index);
...
fn shared_capability_type_tag(shared: &SharedCapability) -> String {
    descriptor_type_tag(shared.saved_cap.descriptor_json.as_deref()).unwrap_or_else(|| match shared
        .kind
    {
...
async fn begin_pair_connection(
    ...
    app_core(&app_state)
        .begin_pair_connection(sandstorm_api)
        .await
}
...
fn parse_claim_payload(value: &str) -> Result<(String, String, Option<String>), String> {
    ...
    if trimmed.starts_with('{') {
        ...
        let label = payload
            ...
            .to_string();
        let descriptor_json = payload.get("descriptor").map(|value| value.to_string());
...
fn encode_powerbox_descriptor(
    ...
    let mut message = capnp::message::Builder::new_default();
    message.set_root(descriptor).map_err(|err| {
        capnp::Error::failed(format!("failed to copy powerbox descriptor: {err}"))
    })?;
    let mut bytes = Vec::new();
    capnp::serialize_packed::write_message(&mut bytes, &message).map_err(|err| {
        capnp::Error::failed(format!("failed to encode powerbox descriptor: {err}"))
    })?;
    Ok(base64::engine::general_purpose::URL_SAFE_NO_PAD.encode(bytes))
}
...
fn decode_pair_control_message(bytes: &[u8]) -> Result<PairControlMessage, String> {
    let mut slice = bytes;
    let reader =
        capnp::serialize_packed::read_message(&mut slice, capnp::message::ReaderOptions::new())
        ...
        .map_err(|err| format!("failed to decode pair control union: {err}"))?
    {
        ...
        tunnel_capnp::pair_control::Response(response) => {
            let response =
                response.map_err(|err| format!("failed to read pair response: {err}"))?;
...
mod tests {
    ...
    fn summarize_endpoint_addr_collects_all_transport_types() {
        ...
        assert_eq!(
            ...
            vec![
                CustomAddr::from_parts(SANDSTORM_RAW_UDP_TRANSPORT_ID, &[0x01, 0x02, 0x03])
                    .to_string()
            ]
        );
    }
    ...
    fn local_ticket_roundtrips_mixed_transports() {
        ...
        assert!(parsed.addrs.contains(&TransportAddr::Relay(
            iroh::RelayUrl::from_str("https://relay.example/").unwrap()
        )));
        assert!(
            parsed
                .addrs
    ...
    fn render_state_json_tracks_live_persisted_disconnected_and_cleared_states() {
        run_local_async_test(async {
            ...
            let initial_state: serde_json::Value =
                serde_json::from_str(&importer_app.render_state_json().unwrap()).unwrap();
            assert_eq!(initial_state["peerRpc"]["connected"], false);
            ...
            let live_state: serde_json::Value =
                serde_json::from_str(&importer_app.render_state_json().unwrap()).unwrap();
            assert_eq!(live_state["peerRpc"]["connected"], true);
            assert_eq!(
                live_state["localProxyCaps"][0]["objectId"],
                "local-proxy-cap-1"
            );
            assert_eq!(live_state["peerRpcError"], serde_json::Value::Null);

            importer_app.disconnect_peer_rpc_session().unwrap();
            let disconnected_state: serde_json::Value =
                serde_json::from_str(&importer_app.render_state_json().unwrap()).unwrap();
            assert_eq!(disconnected_state["peerRpc"]["connected"], false);
            assert_eq!(disconnected_state["peerRpcError"], serde_json::Value::Null);
            ...
            importer_app
                ...
                .await
                .unwrap();
            let cleared_state: serde_json::Value =
                serde_json::from_str(&importer_app.render_state_json().unwrap()).unwrap();
            assert_eq!(cleared_state["peerRpc"]["connected"], true);
            assert_eq!(
                cleared_state["peerRpc"]["capabilityExports"],
                serde_json::json!([])
            );
            ...
            importer_app
                ...
                .unwrap();
            let dropped_state: serde_json::Value =
                serde_json::from_str(&importer_app.render_state_json().unwrap()).unwrap();
            assert_eq!(dropped_state["localProxyCaps"], serde_json::json!([]));
    ...
    fn startup_auto_reconnect_picks_a_single_initiator_after_restart() {
        run_local_async_test(async {
            ...
            let app_a_should_reconnect =
                should_auto_reconnect_tunnel(&restarted_app_a_state).unwrap();
            let app_b_should_reconnect =
                should_auto_reconnect_tunnel(&restarted_app_b_state).unwrap();
    ...
    fn drop_disconnect_and_restore_missing_are_idempotent() {
        run_local_async_test(async {
            ...
            for missing_id in [
                ...
                assert!(
                    err.contains("unknown app object id")
                        || err.contains("unknown saved capability"),
    ...
    fn local_proxy_app_persistent_save_round_trip_restores_same_object() {
        run_local_async_test(async {
            ...
            let (saved_object_id, saved_label) =
                save_app_persistent_capability_for_test(local_proxy)
                    .await
                    .unwrap();
    ...
    fn fulfill_request_with_local_proxy_succeeds_in_fake_session_context() {
        run_local_async_test(async {
            ...
            let capture = Arc::new(Mutex::new(
                crate::test_support::FakeFulfillCapture::default(),
            ));
            let session_context: grain_capnp::session_context::Client =
                new_client(crate::test_support::FakeSessionContextForFulfill {
    ...
    fn brokered_received_api_session_can_be_provided_again_after_both_sides_restart() {
        run_local_async_test(async {
            ...
            let initial_capture = Arc::new(Mutex::new(
                crate::test_support::FakeFulfillCapture::default(),
            ));
            let initial_session_context: grain_capnp::session_context::Client =
                new_client(crate::test_support::FakeSessionContextForFulfill {
            ...
            let restarted_capture = Arc::new(Mutex::new(
                crate::test_support::FakeFulfillCapture::default(),
            ));
            let restarted_session_context: grain_capnp::session_context::Client =
                new_client(crate::test_support::FakeSessionContextForFulfill {
    ...
    fn localized_proxy_for_ephemeral_registered_remote_capability_fails_after_remote_restart() {
        run_local_async_test(async {
            let exporter_storage_root =
                make_test_storage_root("registered-ephemeral-restart-exporter");
            let (exporter_app, exporter_state, exporter_sandstorm_api) =
            ...
            let (
                restarted_exporter_app,
                restarted_exporter_state,
                restarted_exporter_sandstorm_api,
    ...
    fn local_proxy_unwraps_localized_capability_parameters() {
        run_local_async_test(async {
            ...
            let get_echo_resp = get_echo.send().promise.await.unwrap();
            let echo = get_echo_resp.get().unwrap().get_echo().unwrap();
            let echoed = crate::test_support::invoke_echo_relay_for_test(relay, echo, "hello")
                .await
                .unwrap();
...
impl tunnel_capnp::peer_bootstrap::Server for PeerBootstrapImpl {
    fn list_capability_exports(
        ...
        let exports = match self.app_state.lock() {
            Ok(guard) => guard
                ...
                .collect::<Vec<_>>(),
            Err(_) => {
                return Promise::err(capnp::Error::failed("app state lock poisoned".to_string()));
            }
        };
    ...
    fn get_capability_export(
        ...
        Promise::from_future(async move {
            ...
            let cap = match cap {
                Some(cap) => cap,
                None => {
                    let cap = restore_saved_generic_capability(
                        sandstorm_api,
                        &export.saved_cap.saved_token,
                    )
                    .await
    ...
    fn register_capability(
        ...
        let kind = match params.get_kind() {
            ...
            Ok(tunnel_capnp::CapabilityKind::ApiSession) => {
                ImportedRemoteCapabilityKind::ApiSession
            }
        ...
        Promise::from_future(async move {
            let existing = match app_state.lock() {
                Ok(guard) => guard
                    ...
                    .get(&hook_ptr)
                    .cloned(),
                Err(_) => return Err(capnp::Error::failed("app state lock poisoned".to_string())),
            };
            ...
            let saved_token = save_capability(sandstorm_api, client.clone(), &label)
                .await
                .ok();
            ...
            let remote_object_id = {
                ...
                let remote_object_id = format!(
                    "remote-registered-cap-{}",
                    guard.next_registered_remote_cap_id
                );
    ...
    fn get_registered_capability(
        ...
        Promise::from_future(async move {
            ...
            let client = registered.client.clone().ok_or_else(|| {
                capnp::Error::failed("registered capability missing live client".to_string())
            })?;
            let mut result = results.get();
...
async fn run_iroh_accept_loop(
    ...
    while let Some(incoming) = endpoint.accept().await {
        ...
        tokio::task::spawn_local(async move {
            let result = async {
                let mut accepting = incoming.accept().map_err(|err| {
                    format!("failed to begin accepting incoming iroh connection: {err}")
                })?;
                let alpn = accepting
                    .alpn()
                    .await
...
async fn accept_probe_connection(accepting: iroh::endpoint::Accepting) -> Result<(), String> {
...
#[allow(dead_code)]
pub(crate) struct TcpProbeSummary {
...
async fn probe_remote_connection(
    ...
    eprintln!(
        ...
        remote_node_id, remote_addr.addrs
    );
...
async fn finish_saved_ip_network_tcp_exchange(
    ...
    write_to_byte_stream(connection.upstream.clone(), payload).await?;
    {
        let mut trace = connection
            .trace
            .lock()
            .map_err(|_| "tcp session trace lock poisoned".to_string())?;
        trace.push(format!("payload-sent:{}-bytes", payload.len()));
    }
    close_byte_stream(connection.upstream.clone()).await?;
    {
        let mut trace = connection
            .trace
            .lock()
    ...
    loop {
        let notified = {
            ...
            if available > 0 {
                let bytes = incoming.bytes[incoming.read_offset..].to_vec();
                incoming.read_offset = incoming.bytes.len();
                incoming.bytes.clear();
                incoming.read_offset = 0;
                combined_bytes.extend_from_slice(&bytes);
            }
            if incoming.saw_done {
                let trace = connection
                    .trace
                    .lock()
                    .map_err(|_| "tcp session trace lock poisoned".to_string())?
                    .join(" -> ");
                return Ok((combined_bytes, trace));
            }
            if !combined_bytes.is_empty() {
                ...
                return Ok((combined_bytes, trace.join(" -> ")));
            }
            connection.notify.notified()
        };
    ...
    let trace = connection
        ...
        .join(" -> ");
    Err(format!(
        "network exchange timed out before response bytes or stream completion ({trace})"
    ))
}
